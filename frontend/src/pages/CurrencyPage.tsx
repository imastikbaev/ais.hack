  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="page-title">Школьный буфет</h1>
          <p className="text-slate-500 text-sm">1 монета = 1 тенге</p>
        </div>
        {balance && (
      ...
      {orderSuccess && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4">
          ...
          <div className="bg-white rounded-lg p-4 text-center border-2 border-dashed border-green-300">
            <div className="text-xs text-slate-400 mb-1">QR-код для кассы</div>
      ...
      <div className="flex gap-2">
        {[
          { key: 'menu', label: 'Меню', icon: <IconUtensils className="w-4 h-4" /> },
          { key: 'orders', label: 'Мои заказы', icon: <IconClipboard className="w-4 h-4" /> },
        ].map(t => (
          <button key={t.key} onClick={() => setTab(t.key as any)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium ${tab === t.key ? 'bg-primary-600 text-white' : 'bg-slate-100 text-slate-600'}`}>
            {t.icon}
            {t.label}
          </button>
        ))}
      ...
      {tab === 'orders' && (
        <div className="space-y-3">
          {(orders as any[]).length === 0 && (
            <div className="text-center text-slate-400 py-10">Заказов пока нет</div>
          )}
            ...
            <div key={order.id} className="card flex items-center gap-4">
              ...
              {order.status === 'pending' && (
                <div className="text-center">
                  ...
                  <div className="text-xs text-slate-400 mt-0.5">Покажи на кассе</div>
                </div>
              )}
              <span className={`badge ${order.status === 'redeemed' ? 'badge-green' : 'badge-orange'}`}>
                {order.status === 'redeemed' ? 'Получен' : 'Ожидает'}
              </span>
            </div>
          ))}
        </div>
      ...
      {balance?.transactions?.length > 0 && tab === 'menu' && (
        <div className="card">
          <h3 className="font-semibold text-slate-700 mb-3">История транзакций</h3>
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {balance.transactions.map((t: any) => (
              <div key={t.id} className="flex justify-between items-center text-sm py-1.5 border-b border-slate-50">
                ...
                <span className={`font-bold flex items-center gap-1 ${t.amount > 0 ? 'text-green-600' : 'text-red-500'}`}>
                  {t.amount > 0 ? '+' : ''}{t.amount}
